...
Verify:
- `prepare --label <label> [--source {shorthand|file}] [--json]`
...
- `run --label <label> [--source {shorthand|file}] [--status-default {unverified|verified_pass|verified_fail|blocked}] [--json]`

Report:
- `profile (--label <label> | --all) [--json | --csv]`

Promote:
- `--label <label> [--fixture-file <path>] [--overwrite] [--json]`
